...
date: 2017-01-30
coverImage: "malware-evasive-evolving-cybercriminals.jpg"
---
...
**[![](images/Injected-EITest-Script-Final.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/Injected-EITest-Script-Final.png)**

**Second case: Victim’s browser is IE[![](images/Injected-EITest-Script-Second-case-Diagram.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/Injected-EITest-Script-Second-case-Diagram.png)**
...
- January 27th, 2017- Gate server file changed from file.php to downloads.php
    - Payload name changed from chrome\_font.exe to font\_update.exe payload is no longer Fleercivet
...
\[caption id="attachment\_223" align="aligncenter" width="373"\][![](images/Top-5-CMS-infected-Websites.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/Top-5-CMS-infected-Websites.png) Confirmed infected websites. Wordpress: 3171 (55,39%), ? CMS not available: 1894 (33,07%), Joomla: 427 (7,45%)\[/caption\]
...
On a typical Joomla contaminated site, all .PHP files have malware that does the content injection. On WordPress, we’ve seen servers that only wp\_config.php file was affected but in other cases all.PHP files were modified. The malware that infects those PHP files was discovered by [Symantec](https://www.symantec.com/security_response/earthlink_writeup.jsp?docid=2015-111911-4342-99) on November 19, 2015, and haven’t changed much since:

\[caption id="attachment\_224" align="aligncenter" width="2490"\][![](images/Malicious-PHP-content-injector.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/Malicious-PHP-content-injector.png) Obfuscated malicious PHP code injector placed in the header of index.php\[/caption\]
...
\[caption id="attachment\_225" align="aligncenter" width="2148"\][![](images/Deobfuscated-malicious-PHP-code.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/Deobfuscated-malicious-PHP-code.png) Deobfuscated malicious PHP code after the static analysis\[/caption\]
...
After finishing its initial preparation routine, the code will verify if the PHP function **ob\_start** is defined and also if the global variable **anuna** isn’t already set. The injection process will only begin if both these conditions are respected. If it’s the case, it will then set **anuna** to ‘True’ and then proceed with an additional check on the victim’s browser user agent string.
...
\[caption id="attachment\_228" align="aligncenter" width="1964"\][![](images/initial-verification.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/initial-verification.png) The initial verification steps before continuing with content injection\[/caption\]
...
It all relies on the **ob\_start** function, explaining why its existence is the first thing being verified. This function is basically used to enable output buffering, which prevent any output (except headers) from being sent by the script, instead of storing everything in an internal buffer. Usually the buffer content can be retrieved by calling any of the methods to flush it, but in this case, only **ob\_start** is called, nothing else. The trick resides in the fact that **ob\_start** can also be called with the name of a callback function as an argument. This means that the buffer content is automatically returned as an argument of the provided function name when it is about to be flushed. This happens when the request ends. To sum it up, anything outputted after **ob\_start** is called will be sent to another function when the request is finishing.
...
\[caption id="attachment\_229" align="aligncenter" width="1364"\][![](images/entry-point-function.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/entry-point-function.png) The entry-point function receiving the page content\[/caption\]

The function starts by setting the **Content-Encoding** header to none, then proceed to decompress the page content using a custom decoding function named **do()**. If the content isn’t compressed, it is returned to its original state. By pairing **preg\_match** and **preg\_replace** the script is able to locate where the malicious payload should be injected into the page. The resulting HTML is then outputted to the page because **pa22()** returns from the output buffer.

**_Now, how the actual payload is retrieved before being injected?_** The **day212()** function is the culprit!

\[caption id="attachment\_230" align="aligncenter" width="2088"\][![](images/core-deobfuscated-function.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/core-deobfuscated-function.png) The core deobfuscated function\[/caption\]
...
- The User-Agent matches a known crawler (Google, Slurp, MsnBot, IE\_Archiver, Yandex, Rambler)
...
\[caption id="attachment\_231" align="aligncenter" width="619"\][![](images/crafted-url.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/crafted-url.png) Crafted URL example\[/caption\]
...
[![](images/sinkholed.png)](https://blog.brillantit.com/wp-content/uploads/2017/02/sinkholed.png)
...
\[caption id="attachment\_451" align="aligncenter" width="775"\][![](images/Unsuccessful_call_censored.jpg)](https://blog.brillantit.com/wp-content/uploads/2017/01/Unsuccessful_call_censored.jpg) Unsuccessful call made to EITest gate to obtain the injection content\[/caption\]
...
\[caption id="attachment\_233" align="aligncenter" width="842"\][![](images/Successful-call_censored-.jpg)](https://blog.brillantit.com/wp-content/uploads/2017/01/Successful-call_censored-.jpg) Successful call made to EITest gate to get the malicious injection\[/caption\]
...
\[caption id="attachment\_236" align="aligncenter" width="738"\][![](images/main-method-using-for-encrypting.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/main-method-using-for-encrypting.png) The main method using for encrypting/decrypting communications\[/caption\]

It seems like a custom implementation of the **HMAC\_MD5** algorithm, with **q1w2e3r4** as the key. Even if this algorithm relies on an MD5 hash, there is no known way of breaking it in its current state, so the fact that we were able to grab the key helped us a lot.
...
[![](images/step-1-decryption.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/step-1-decryption.png)
...
\[caption id="attachment\_238" align="aligncenter" width="1328"\][![](images/step-2.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/step-2.png) python injPayloadDecryter.py HEXSTRING OPERATIONID\[/caption\]
...
[![](images/returned-content.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/returned-content.png)
...
[![](images/PHP-content-injector-flow.jpg)](https://blog.brillantit.com/wp-content/uploads/2017/01/PHP-content-injector-flow.jpg)
...
\[caption id="attachment\_377" align="alignnone" width="844"\][![](images/fake_website.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/fake_website.png) "Fake" breaking news website hosted on each 4 of the domains\[/caption\]
...
\[caption id="attachment\_240" align="aligncenter" width="891"\][![](images/font-not-found.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/font-not-found.png) The "HoeflerText font wasn’t found" message\[/caption\]
...
\[caption id="attachment\_251" align="aligncenter" width="1351"\][![](images/Download-of-Fleercivet.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/Download-of-Fleercivet.png) Downloading the malware after clicking on 'Update.'\[/caption\]
...
\[caption id="attachment\_252" align="aligncenter" width="1322"\][![](images/‌Injected-content.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/‌Injected-content.png) Injected content asking the user to download chrome\_font.exe\[/caption\]
...
\[caption id="attachment\_411" align="aligncenter" width="961"\][![](images/Fleercivet-flow-chart.jpg)](https://blog.brillantit.com/wp-content/uploads/2017/01/Fleercivet-flow-chart.jpg) Simplified overview of the traffic flow leading to Fleercivet infection\[/caption\]
...
\[caption id="attachment\_254" align="aligncenter" width="1461"\][![](images/Hash-change-over-time.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/Hash-change-over-time.png) Each dot represent a new hash seen on the back-end server\[/caption\]
...
\[caption id="attachment\_255" align="aligncenter" width="835"\][![](images/fleecivert-process-tree.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/fleecivert-process-tree.png) Process tree created by Chrome\_font.exe\[/caption\]
...
\[caption id="attachment\_256" align="aligncenter" width="815"\][![](images/help-PHP-file.jpg)](https://blog.brillantit.com/wp-content/uploads/2017/01/help-PHP-file.jpg) The help.php file\[/caption\]
...
\[caption id="attachment\_257" align="aligncenter" width="786"\][![](images/POST-to-download_censored.jpg)](https://blog.brillantit.com/wp-content/uploads/2017/01/POST-to-download_censored.jpg) POST request made by one victim to download.php\[/caption\]

[Using this python script](https://blog.brillantit.com/wp-content/uploads/2017/01/infol_Decrypter.txt) will decrypt the information sent:
...
\[caption id="attachment\_259" align="aligncenter" width="1742"\][![](images/log-file_censored.jpg)](https://blog.brillantit.com/wp-content/uploads/2017/01/log-file_censored.jpg) Logo2.png file containing the logs\[/caption\]
...
\[caption id="attachment\_320" align="aligncenter" width="1174"\][![](images/top-100-redirected-websites_censored.jpg)](https://blog.brillantit.com/wp-content/uploads/2017/01/top-100-redirected-websites_censored.jpg) Top infected websites that redirected users to help.php\[/caption\]

\[caption id="attachment\_262" align="aligncenter" width="497"\][![](images/TOP-30-TLD_1.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/TOP-30-TLD_1.png) Confirmed infected website .com: 2599(48.26%), .uk: 344(6.39%), .org: 330(6.13%), .it: 323(6%), .fr: 233 (4.33%)\[/caption\]
...
\[caption id="attachment\_263" align="aligncenter" width="1306"\][![](images/Heatmap.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/Heatmap.png) Heat map showing the location of users that requested the malware to a server hosting help.php\[/caption\]

\[caption id="attachment\_264" align="aligncenter" width="685"\][![](images/top-redirected-country-.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/top-redirected-country-.png) US: 10743(37.19%), UK: 4173(14.45%), CAN: 2320(8.03%), FR :1913 (6.62%)\[/caption\]

Statistics about the user agent of redirected users were also made:

[![](images/Top-UA.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/Top-UA.png)
...
\[caption id="attachment\_266" align="aligncenter" width="974"\][![](images/Malicious-Iframe-1.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/Malicious-Iframe-1.png) Malicious injected iframe 1 redirecting to RIG EK\[/caption\]

\[caption id="attachment\_267" align="aligncenter" width="901"\][![](images/Malicious-Iframe-2.png.jpg)](https://blog.brillantit.com/wp-content/uploads/2017/01/Malicious-Iframe-2.png.jpg) Malicious injected iframe 2 redirecting to RIG EK\[/caption\]
...
\[caption id="attachment\_268" align="aligncenter" width="2238"\][![](images/First-response.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/First-response.png) First response (Verification Script) from the RIG exploit kit\[/caption\]

**http://try.ciela.co** resolves to **92.53.120.14** (MOSCOW) and is running NGINX.

\[caption id="attachment\_269" align="aligncenter" width="1858"\][![](images/first-verification-entry-point.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/first-verification-entry-point.png) First verification script entry point\[/caption\]
...
\[caption id="attachment\_323" align="aligncenter" width="2500"\][![](images/obfuscated-Javascript.jpg)](https://blog.brillantit.com/wp-content/uploads/2017/01/obfuscated-Javascript.jpg) The obfuscated Javascript returned after form submission\[/caption\]
...
\[caption id="attachment\_270" align="aligncenter" width="2016"\][![](images/main-function.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/main-function.png) The main function used to abuse Adobe Flash Player in IE\[/caption\]
...
[![](images/java-hexa.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/java-hexa.png)
...
\[caption id="attachment\_274" align="aligncenter" width="674"\][![](images/shellcode.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/shellcode.png) The shellcode after reaching its entry point\[/caption\]

If you are familiar with x86 ASM and common patterns you can see that this is a simple loop used to decrypt a memory blob using XOR and 84 as a key. By putting a breakpoint at **0x401012**, we will pause the execution right after the decryption loop exited. This gives us the payload in plain text. By combining this new information with what we previously gathered we now have a clear picture of the last step in this infection chain.[![](images/payload.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/payload.png)
...
\[caption id="attachment\_276" align="aligncenter" width="2380"\][![](images/VBScript-generated.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/VBScript-generated.png) The obfuscated VBScript generated by the shellcode\[/caption\]
...
\[caption id="attachment\_277" align="aligncenter" width="2240"\][![](images/deobfuscated-VBScript-shellcode.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/deobfuscated-VBScript-shellcode.png) The deobfuscated VBScript shellcode\[/caption\]
...
[![](images/cerber-temp.jpg)](https://blog.brillantit.com/wp-content/uploads/2017/01/cerber-temp.jpg)

tmpBEEC.bmp is, in fact, the ransom note that Cerber is about to put as the wallpaper on the infected computer.

[![](images/Cerber_censored.jpg)](https://blog.brillantit.com/wp-content/uploads/2017/01/Cerber_censored.jpg)
...
\[caption id="attachment\_280" align="aligncenter" width="1142"\][![](images/cryptomix.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/cryptomix.png) Ransomware note left after encryption done is by CryptoMix\[/caption\]
...
- Three .DLL files are downloaded to make the bot gain all of its functionality: [![](images/dslls.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/dslls.png) 

\[caption id="attachment\_282" align="aligncenter" width="968"\][![](images/malicious-.dll-.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/malicious-.dll-.png) Accessing directly the repertory of the malicious .dll files hosted on 195.161.62.33\[/caption\]
...
[![](images/Botnet-in.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/Botnet-in.png)
...
\[caption id="attachment\_284" align="aligncenter" width="1313"\][![](images/Apache-Status_censored.jpg)](https://blog.brillantit.com/wp-content/uploads/2017/01/Apache-Status_censored.jpg) Bots reporting to the C&C server via GET request\[/caption\]

This showed us that many bots were also checking-in. By monitoring this page, we were also able to spot more unusual requests, like an administrator logging in the admin panel from a TOR exit node:[![](images/admin.jpg)](https://blog.brillantit.com/wp-content/uploads/2017/01/admin.jpg) Also, we were able to make the statistic about bots who were checking in:

\[caption id="attachment\_286" align="aligncenter" width="1643"\][![](images/Botnet-GET.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/Botnet-GET.png) Each check in GET request made to the C&C madness server over time (UTC)\[/caption\]
...
\[caption id="attachment\_287" align="aligncenter" width="784"\][![](images/TOP-OS.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/TOP-OS.png) the operating system for unique bot ID\[/caption\]
...
\[caption id="attachment\_288" align="aligncenter" width="968"\][![](images/Madness-Control-Center.png)](https://blog.brillantit.com/wp-content/uploads/2017/01/Madness-Control-Center.png) Madness login panel page version 03.10.2016\[/caption\]
...
[![](images/1580-bots_censored.jpg)](https://blog.brillantit.com/wp-content/uploads/2017/01/1580-bots_censored.jpg)
